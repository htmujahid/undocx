function QuizVisual() {
  const options = [
    {
      label: "A",
      text: "It produces ATP through glycolysis only",
      correct: false,
    },
    {
      label: "B",
    ...
    {
      label: "C",
      text: "It breaks down glucose into carbon dioxide",
      correct: false,
    },
    {
      label: "D",
      text: "It occurs exclusively in animal cells",
      correct: false,
    },
  ]
      ...
      <div className="px-4 py-3">
        ...
        <div className="mt-3 space-y-1.5">
          {options.map((opt) => (
            ...
            >
              <span
                className={`flex size-4 shrink-0 items-center justify-center rounded-full text-[9px] font-bold ${opt.correct ? "bg-primary text-primary-foreground" : "bg-muted"}`}
              >
                {opt.label}
              </span>
              {opt.text}
        ...
        {columns.map((col) => (
          <div key={col.title} className="flex-1 min-w-0">
            <div className="mb-1.5 flex items-center gap-1">
              ...
              <p
                className={`text-[9px] font-semibold uppercase tracking-wide ${col.color}`}
              >
                {col.title}
              </p>
            </div>
            <div className="space-y-1">
              {col.items.map((item) => (
                <div
                  key={item}
                  className="rounded-md border border-border bg-background px-2 py-1.5 text-[10px] leading-tight text-foreground shadow-sm"
                >
                  {item}
                </div>
              ))}
        ...
        <div className="mt-3 space-y-1">
          <div className="grid grid-cols-4 gap-1.5">
            {["React", "Vue", "Svelte", "Angular"].map((h) => (
              <div
                key={h}
                className="rounded bg-primary/8 px-1.5 py-1 text-center text-[9px] font-semibold text-primary"
              >
                {h}
              </div>
            ))}
          ...
          {[
            ...
            <div key={i} className="grid grid-cols-4 gap-1.5">
              {row.map((cell, j) => (
                <div
                  key={j}
                  className="rounded bg-muted/50 px-1.5 py-1 text-center text-[9px] text-muted-foreground"
                >
                  {cell}
                </div>
              ))}
