...
import { INSERT_INLINE_COMMENT_COMMAND } from "./comment-plugin";
...
function FloatingTextFormat({
  ...
  const { documentMode } = useEditorContext();
  ...
  // In viewing mode, don't show the toolbar at all
  if (documentMode === "viewing") {
    return <></>;
  }

  // In commenting mode, only show the comment button
  if (documentMode === "commenting") {
    return (
      <div
        ref={popupCharStylesEditorRef}
        className="bg-background absolute top-0 left-0 flex gap-1 rounded-lg border p-1 opacity-0 shadow-md transition-opacity duration-300 will-change-transform"
      >
  ...
  // In editing mode, show all formatting options
  return (
    <div
      ref={popupCharStylesEditorRef}
      className="bg-background absolute top-0 left-0 flex gap-1 rounded-lg border p-1 opacity-0 shadow-md transition-opacity duration-300 will-change-transform"
    >
      ...
      <ToggleGroup
        type="multiple"
        defaultValue={[isCode ? "code" : "", isLink ? "link" : ""]}
      >
        <ToggleGroupItem
          value="code"
